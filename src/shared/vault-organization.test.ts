...
it('preserves unrelated expanded branches during rename and move', () => {
  ...
  const changed: VaultEntry = { kind: 'folder', path: 'Work', name: 'Work', children: [{ kind: 'canvas', path: 'Work/Canvas.yantraC', name: 'Canvas.yantraC' }] };
  const renamed = relocateEntries([untouched, changed], 'Work/Canvas.yantraC', 'Work/Renamed.yantraC');
  expect(renamed.find((entry) => entry.path === 'Other')).toBe(untouched);
  expect(renamed.find((entry) => entry.path === 'Work')?.children?.[0]?.path).toBe('Work/Renamed.yantraC');
  const moved = relocateEntries([untouched, changed], 'Work', 'Renamed');
  expect(moved.find((entry) => entry.path === 'Other')).toBe(untouched);
  expect(moved.find((entry) => entry.path === 'Renamed')?.children?.[0]?.path).toBe('Renamed/Canvas.yantraC');
});
