...
export function createMarkdownNodeAt(position) {
...
function getNodeWidth(node) {
  return toCanvasInteger(
    Number(node.width ?? node.measured?.width ?? node.style?.width),
    NODE_WIDTH,
  );
}
...
function getNodeHeight(node) {
  return toCanvasInteger(
    Number(node.height ?? node.measured?.height ?? node.style?.height),
    NODE_HEIGHT,
  );
}
...
export function toJsonCanvasDocument(state) {
  return {
...
export function hydrateEdges(edges) {
